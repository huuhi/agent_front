...
<template>
  <aside class="w-[260px] min-w-[260px] bg-white border-r border-stone-100 flex flex-col h-full">
    ...
    <div class="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
      <div v-if="loading" class="flex items-center justify-center py-8 text-xs text-stone-400">加载中...</div>
      <div v-else-if="errorMsg" class="px-3 py-4 text-xs text-center text-red-500">{{ errorMsg }}</div>
      <div v-else-if="sessions.length === 0" class="px-3 py-8 text-xs text-center text-stone-400">暂无历史对话</div>
      <div v-for="session in sessions" :key="session.id"
        ...
        :class="currentSessionId === session.id ? 'bg-violet-50 text-violet-700 font-medium' : 'text-stone-600 hover:bg-stone-50 hover:text-stone-800'"
      >
        <svg class="w-4 h-4 shrink-0" :class="currentSessionId === session.id ? 'text-violet-500' : 'text-stone-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
        ...
        <button v-else
          ...
          class="shrink-0 p-1 rounded-lg opacity-0 group-hover:opacity-100 text-stone-400 hover:text-red-500 hover:bg-red-50 transition-all duration-150" title="删除对话"
    ...
    <div class="border-t border-stone-100 px-3 py-3">
      <div class="flex items-center gap-3 px-2 mb-3">
        <svg class="w-8 h-8 shrink-0" viewBox="0 0 32 32" fill="none">
          <rect width="32" height="32" rx="16" fill="#78716C"/>
      ...
      <div class="border-t border-stone-100 pt-2 space-y-1">
        ...
        <button @click="emit('openAPIConfig')" class="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm text-stone-500 hover:text-stone-700 hover:bg-stone-50 transition-all duration-150">
