...
function onPasteFile(e: ClipboardEvent) {
  ...
  <div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white border border-stone-100 rounded-2xl shadow-md transition-shadow duration-200 focus-within:shadow-lg relative">
      <textarea :value="inputText" @input="onTextareaInput"
        ...
        class="w-full resize-none bg-transparent px-5 pt-4 pb-2 text-sm text-stone-800 placeholder-stone-400 focus:outline-none leading-relaxed chat-textarea"
      ></textarea>

      <!-- Uploaded file previews -->
      <div v-if="uploadedPreviews.length > 0 || uploadingCount > 0" class="flex flex-wrap gap-2 px-4 pb-2">
        <!-- Uploading indicator -->
        <div v-if="uploadingCount > 0" class="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-stone-50 border border-stone-100 text-[11px] text-stone-400">
          <div class="w-3 h-3 rounded-full border-2 border-violet-200 border-t-violet-500 animate-spin"></div>
          <span>上传中...</span>
        </div>
        <!-- Preview cards -->
        <div v-for="(att, fi) in uploadedPreviews" :key="att.id"
          class="flex items-center gap-1.5 rounded-lg border border-stone-100 overflow-hidden"
          :class="att.type === 'image' ? 'p-0.5 bg-stone-50' : 'px-2.5 py-1.5 bg-stone-50'"
        >
          <!-- Image thumbnail -->
          ...
          />
          <!-- Document icon -->
          <svg v-else class="w-4 h-4 shrink-0 text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
          </svg>
          <span class="text-stone-600 truncate max-w-[100px] text-[11px]">{{ att.name }}</span>
          <span class="text-stone-400 text-[10px]">{{ formatFileSize(att.size) }}</span>
          <button @click="emit('removePreview', fi)" class="text-stone-400 hover:text-red-500 ml-0.5 transition-colors">
      ...
      <div class="flex items-center justify-between px-4 pb-3">
        <div class="flex items-center gap-0.5">
          <button @click.stop="emit('toggleThinking')"
            ...
            :class="enableThinking ? 'text-violet-600 bg-violet-50' : 'text-stone-400 hover:text-stone-600 hover:bg-stone-50'"
          >
          ...
          <div ref="optionsPanelRef" class="relative">
            <button @click.stop="toggleOptions"
              ...
              :class="showOptions ? 'bg-stone-100 text-stone-600' : 'text-stone-400 hover:text-stone-600 hover:bg-stone-50'"
              title="更多选项"
            >
            ...
            <transition name="dropdown">
              <div v-if="showOptions" class="absolute bottom-full mb-1 left-0 w-80 bg-white border border-stone-100 rounded-xl shadow-lg z-50">
                <div class="py-2 max-h-[50vh] overflow-y-auto">
                  <!-- API Config -->
                  <div class="px-4 py-2 border-b border-stone-50">
                    <div class="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-1.5">API 配置</div>
                    <div class="relative">
                      <button ref="apiConfigBtnRef" @click.stop="toggleApiConfig($event)"
                        class="w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-stone-100 text-xs text-stone-700 hover:border-stone-200 transition-all"
                      >
                        <span class="flex-1 text-left truncate">{{ selectedApiConfig ? (selectedApiConfig.name || selectedApiConfig.baseUrl) : '系统默认' }}</span>
                        <svg class="w-3 h-3 text-stone-300 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
                      </button>
                    </div>
                  </div>
                  ...
                  <!-- Model -->
                  <div class="px-4 py-2 border-b border-stone-50">
                    <div class="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-1.5">模型</div>
                    <div v-if="!selectedApiConfig" class="px-3 py-2 rounded-lg text-xs text-stone-400 bg-stone-50 border border-stone-100">
                      系统默认
                    </div>
                  ...
                  <!-- Knowledge Base -->
                  <div class="px-4 py-2 border-b border-stone-50">
                    <div class="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-1.5">知识库</div>
                    <div class="relative">
                  ...
                  <!-- MCP Servers -->
                  <div v-if="mcps.length > 0" class="px-4 py-2 border-b border-stone-50">
                    <div class="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-1.5">MCP 服务器</div>
                    <div class="relative">
                  ...
                  <div class="px-4 py-2">
                    <button @click.stop="emit('toggleRag')"
                      ...
                      :class="enableRag ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'text-stone-400 hover:text-stone-600 hover:bg-stone-50 border border-transparent hover:border-stone-100'"
                    >
                      <svg class="w-3.5 h-3.5" :class="enableRag ? 'text-emerald-500' : 'text-stone-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                      <span class="flex-1 text-left">RAG 知识库检索</span>
                      <span class="text-[10px]" :class="enableRag ? 'text-emerald-500' : 'text-stone-300'">{{ enableRag ? '已开启' : '已关闭' }}</span>
                    </button>
                  </div>
                </div>
              </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <button @click="handleAttachClick" class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-stone-400 hover:text-stone-600 hover:bg-stone-50 transition-all duration-150" title="上传附件">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg>
          </button>
          <span v-if="isAiResponding" class="text-xs text-stone-400 animate-pulse">AI 响应中...</span>
          <div class="relative w-9 h-9">
            ...
            <button v-else @click="emit('sendMessage')"
              ...
              :class="(inputText.trim() || uploadedPreviews.length > 0) ? 'bg-emerald-500 text-white hover:bg-emerald-600 shadow-sm' : 'bg-stone-100 text-stone-300'"
              :disabled="!inputText.trim() && uploadedPreviews.length === 0"
    ...
    <div v-if="showApiConfigOptions" :style="apiConfigOverlayStyle" class="bg-white border border-stone-100 rounded-lg shadow-lg z-[999] overflow-hidden" @click.stop>
      <div class="py-1 max-h-[200px] overflow-y-auto">
        <div @click="selectApiConfig(null)" class="px-3 py-2 text-xs cursor-pointer transition-colors" :class="!selectedApiConfig ? 'bg-violet-50 text-violet-700 font-medium' : 'text-stone-600 hover:bg-stone-50'">系统默认</div>
    ...
    <div v-if="showKnowledgeOptions" :style="knowledgeOverlayStyle" class="bg-white border border-stone-100 rounded-lg shadow-lg z-[999] overflow-hidden" @click.stop>
      <div class="py-1 max-h-[200px] overflow-y-auto">
        <div @click="selectKnowledgeBase(null)" class="px-3 py-2 text-xs cursor-pointer transition-colors" :class="!selectedKnowledgeBase ? 'bg-violet-50 text-violet-700 font-medium' : 'text-stone-600 hover:bg-stone-50'">无知识库</div>
        <div v-for="kb in knowledgeBases" :key="kb.id"
          @click="selectKnowledgeBase(kb)"
          class="px-3 py-2 text-xs cursor-pointer transition-colors"
          :class="selectedKnowledgeBase?.id === kb.id ? 'bg-violet-50 text-violet-700 font-medium' : 'text-stone-600 hover:bg-stone-50'"
        >
          <div class="font-medium">{{ kb.name }}</div>
          <div class="text-[10px] text-stone-400 mt-0.5">{{ kb.describe || '暂无描述' }}</div>
        </div>
      </div>
    </div>

    <!-- MCP sub-dropdown -->
    <div v-if="showMCPOptions" :style="mcpOverlayStyle" class="bg-white border border-stone-100 rounded-lg shadow-lg z-[999] overflow-hidden" @click.stop>
      <div class="py-1 max-h-[200px] overflow-y-auto">
        <label v-for="mcp in mcps" :key="mcp.id"
          class="flex items-center gap-2.5 px-3 py-2 cursor-pointer transition-colors hover:bg-stone-50 text-xs"
          :class="!mcp.available ? 'opacity-40' : ''"
        >
          <input type="checkbox" :checked="selectedMCPIds.includes(mcp.id)"
            @change="emit('toggleMCP', mcp.id)"
            class="w-3.5 h-3.5 rounded border-stone-300 text-violet-500 focus:ring-violet-400"
          />
          <span class="flex items-center gap-1.5 text-stone-700">
            <span class="w-1.5 h-1.5 rounded-full" :class="mcp.available ? 'bg-emerald-400' : 'bg-stone-300'"></span>
            {{ mcp.name }}
          </span>
        </label>
      </div>
    ...
    <!-- Image lightbox -->
    <div v-if="previewImage" @click="previewImage = ''" class="fixed inset-0 z-[9999] bg-black/60 flex items-center justify-center p-4 cursor-pointer">
      <img :src="previewImage" class="max-w-full max-h-full rounded-xl shadow-2xl" @click.stop />
    </div>
  </div>
</template>
