...
// ── Data ──
const allMemories = ref<UserMemoryVO[]>([])
const initialLoading = ref(false)
const searchKey = ref('')
const deletingId = ref<string | number | null>(null)

// Local filter — zero network cost
const filteredMemories = computed(() => {
  const q = searchKey.value.trim().toLowerCase()
  if (!q) return allMemories.value
  return allMemories.value.filter(m => m.content.toLowerCase().includes(q))
})
...
const activeMemory = computed(() => {
  if (activePopoverId.value === null) return null
  return allMemories.value.find(m => m.id === activePopoverId.value) ?? null
})

function openPopover(id: string | number, event: MouseEvent) {
...
async function loadAllMemories() {
  ...
  try {
    allMemories.value = await fetchUserMemories()
  } catch {
    allMemories.value = []
  } finally {
...
function onDocumentClick(e: MouseEvent) {
  if (activePopoverId.value !== null) {
    ...
    if (!target.closest('.memory-popover-trigger') && !target.closest('.memory-popover-menu')) {
      closePopover()
    }
  }
}
...
watch(() => props.visible, (v) => {
  if (v) {
    searchKey.value = ''
    closePopover()
    if (allMemories.value.length === 0) loadAllMemories()
  }
})
</script>
...
<template>
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="visible" class="fixed inset-0 z-[9999] flex items-start justify-center pt-[8vh]">
        <!-- Backdrop — click to close -->
        <div class="absolute inset-0 bg-black/30 backdrop-blur-sm cursor-pointer" @click="emit('close')"></div>

        <!-- Panel — no @click.stop so clicks bubble to document for popover close -->
        <div class="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-stone-100 flex flex-col max-h-[75vh]">
          ...
          <div class="flex-1 overflow-y-auto px-5 py-4 space-y-3">
            ...
            <!-- Empty -->
            <div v-else-if="filteredMemories.length === 0" class="text-center py-10">
            ...
            <!-- Memory Cards (render from filteredMemories) -->
            <div v-for="mem in filteredMemories" :key="mem.id" class="relative group">
              <div class="bg-violet-50/70 border border-violet-100/60 rounded-xl px-4 py-3.5 transition-all duration-150 hover:border-violet-200/80">
                <div class="flex items-start gap-3">
                  ...
                  <!-- 3-dot trigger — group-hover reveal -->
                  <button
                    @click="openPopover(mem.id, $event)"
                    class="memory-popover-trigger shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-stone-400 hover:text-stone-600 hover:bg-violet-100/60 transition-all duration-150 opacity-0 group-hover:opacity-100"
                  >
    ...
    <!-- ====== Popover Menu (Teleported to body) ====== -->
    <Teleport to="body">
      <Transition name="popover">
        <div
          v-if="activePopoverId !== null"
          class="memory-popover-menu fixed z-[10000] w-56 bg-white rounded-xl shadow-lg border border-stone-200 py-1.5 overflow-hidden"
          :style="{ top: popoverPos.top + 'px', left: popoverPos.left + 'px' }"
          @click.stop
        >
          <template v-if="activeMemory">
            <button
              @click="handleDelete(activeMemory.id)"
              :disabled="deletingId === activeMemory.id"
            ...
            >
              ...
              <span>{{ deletingId === activeMemory.id ? '删除中...' : '删除' }}</span>
            </button>
            ...
            <div class="px-4 py-2 text-xs text-stone-400 space-y-0.5">
              <div class="flex items-center gap-1">
                ...
                <span>{{ formatDate(activeMemory.createAt) }}</span>
              </div>
              <div v-if="activeMemory.source" class="flex items-center gap-1">
                ...
                <span>保存来源：</span>
                <button
                  @click="handleSourceClick(activeMemory.source!)"
...
/* Popover (rendered as direct child of body, scoped styles won't apply) */
</style>
